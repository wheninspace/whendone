...
| Per resume (additionally) | + `references/resume.md` (1,355 cl100k tokens, re-measured 2026-07-19 after the resume publish-order fix (M6: render+publish moved from step 4 to step 5, after the status flip), up from 1,211 after the Source-B/C resume-routing fix — read ONLY when resuming) + ~0.9–1.5k for the full artifact rebuild (the old session's scratchpad file is gone). Accepted trade-off: a resume loads SKILL.md core *plus* resume.md — a small net rise on the rare resume path so that every non-resume trigger (fresh job, ETA question, accuracy report, stop) drops ~0.9k |
